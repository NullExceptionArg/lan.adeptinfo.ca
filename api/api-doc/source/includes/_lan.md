...
Obtenir le LAN courant, soit celui qui s'affichera par défaut quand les utilisateurs visiteront le site.

### Requête HTTP

`GET /api/lans/current`

### Query Params

Paramètre | Description | Règles de validation
--------- | ----------- | --------------------
fields | Liste des champs à obtenir pour le LAN courant, séparés par des virgules. Si ce paramètre est laissé vide, le LAN au complet sera retourné. Voir champs disponibles | Aucune.
...
## Mettre à jour le LAN courant

Mettre à le LAN courant, soit celui qui s'affichera par défaut quand les utilisateurs visiteront le site.

### Requête HTTP

`POST /api/lan/<lan_id>/current`
...
Paramètre | Description | Règles de validation
--------- | ----------- | --------------------
lan_id | Id du LAN que l'administrateur veut modifier comme courant. | Requis, string.
